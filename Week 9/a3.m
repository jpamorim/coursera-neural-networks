...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  size_input = size(data.inputs, 2);
  deriv_softmax = class_prob - data.targets;
  deriv_log = hid_output .* (1 - hid_output);
  deriv_log_input = model.hid_to_class' * deriv_softmax .* deriv_log;

  classification_loss_1 = deriv_log_input * data.inputs' ./ size_input;
  classification_loss_2 = deriv_softmax * hid_output' ./ size_input; 
  
  wd_loss_1 = wd_coefficient * model.input_to_hid;
  wd_loss_2 = wd_coefficient * model.hid_to_class;
  
  ret.input_to_hid = classification_loss_1 + wd_loss_1;
  ret.hid_to_class = classification_loss_2 + wd_loss_2;
end
